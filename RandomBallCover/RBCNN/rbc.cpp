...
void buildExact(matrix x, matrix *r, rep *ri, unint numReps){
  ...
  r->mat = (float*)calloc( sizeOfMat(*r), sizeof(*r->mat) );
  ...
  float *dToReps = (float*)calloc(x.pr, sizeof(*dToReps));
  ...
  float **tempD = (float**)calloc(numReps, sizeof(*tempD));
  for(i=0; i<numReps; i++){
    tempI[i] = (unint*)calloc(ri[i].len, sizeof(**tempI));
    tempD[i] = (float*)calloc(ri[i].len, sizeof(**tempD));
    ri[i].lr = (unint*)calloc(ri[i].len, sizeof(*ri[i].lr));
    ri[i].dists = (float*)calloc(ri[i].len, sizeof(*ri[i].dists));
...
void searchExact(matrix q, matrix x, matrix r, rep *ri, unint *NNs, float *dToNNs){
  unint i, j, k;
  unint *repID = (unint*)calloc(q.pr, sizeof(*repID));
  float *dToReps = (float*)calloc(q.pr, sizeof(*dToReps));
...
void searchExactK(matrix q, matrix x, matrix r, rep *ri, unint **NNs, float **dNNs, unint K){
  unint i, j, k;
  unint *repID = (unint*)calloc(q.pr, sizeof(*repID));
  float **dToReps = (float**)calloc(q.pr, sizeof(*dToReps));
  for(i=0; i<q.pr; i++)
    dToReps[i] = (float*)calloc(K, sizeof(**dToReps));
  intList *toSearch = (intList*)calloc(r.pr, sizeof(*toSearch));
  for(i=0;i<r.pr;i++)
    createList(&toSearch[i]);
  int nt = omp_get_max_threads();
...
void searchExactManyCores(matrix q, matrix x, matrix r, rep *ri, unint *NNs, float *dToNNs){
  unint i, j, k;
  unint *repID = (unint*)calloc(q.pr, sizeof(*repID));
  float *dToReps = (float*)calloc(q.pr, sizeof(*dToReps));
  intList *toSearch = (intList*)calloc(r.pr, sizeof(*toSearch));
...
// with a high core count (say > 4)
void searchExactManyCoresK(matrix q, matrix x, matrix r, rep *ri, unint **NNs, float **dNNs, unint K){
  unint i, j, k;
  unint **repID = (unint**)calloc(q.pr, sizeof(*repID));
  for(i=0; i<q.pr; i++)
...
void buildOneShot(matrix x, matrix *r, rep *ri, unint numReps){
  ...
  float **dToNNs = (float**)calloc(r->pr, sizeof(*dToNNs));
  for( i=0; i<r->pr; i++){
    repID[i] = (unint*)calloc(CPAD(ps), sizeof(**repID));
    dToNNs[i] = (float*)calloc(CPAD(ps), sizeof(**dToNNs));
  }
  ...
  for( i=0; i<r->pr; i++){
    ...
    ri[i].dists = (float*)calloc(ps, sizeof(*ri[i].dists));
    ri[i].len = s;
    for (j=0; j<s; j++){
      ri[i].lr[j] = repID[i][j];
    }
...
void searchOneShotK(matrix q, matrix x, matrix r, rep *ri, unint **NNs, float **dNNs, unint K){

  unint *repID = (unint*)calloc(q.pr, sizeof(*repID));
  float *dT = (float*)calloc(q.pr, sizeof(*dT));
...
void pickReps(matrix x, matrix *r){
  ...
  unsigned int processMilisec = clock();
  klVector<int> perm= kl_random_permutation<int>(n, processMilisec );
