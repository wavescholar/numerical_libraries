...
void printVector(float *x, unint d){
  unint i;
...
void copyVector(float *x, float *y, unint d){
  unint i;
...
double timeDiff(struct timeRBC start, struct timeRBC end){
...
// array with MAX_float and the sortInds with DUMMY_IDX.  
// This destroys the heap.
void heapSort(heap *hp, unint *sortInds, float *sortVals){
  unint i,j, swapInd;
  heapEl t;

  for(i=0; i<hp->len; i++){
    sortVals[i] = MAX_float;
