...
void writeNeighbs(char*,char*,unint**,float**);
...
int mainOneShotDriver(int argc, char**argv)
{
  ...
  float **dNNs = (float**)calloc( m, sizeof(*dNNs) ); //dists to NNs
  for(i=0;i<m; i++){
    NNs[i] = (unint*)calloc(K, sizeof(**NNs));
    dNNs[i] = (float*)calloc(K, sizeof(**dNNs) );
  }
  ...
  if(runBrute){
    ...
    float **dToNNsBrute = (float**)calloc( m, sizeof(*dToNNsBrute) );;
    for(i=0; i<m; i++){
      NNsBrute[i] = (unint*)calloc( K, sizeof(**NNsBrute) );
      dToNNsBrute[i] = (float*)calloc( K, sizeof(**dToNNsBrute) );
    }
    gettimeofday(&tvB,NULL);
...
void readData(char *dataFile, matrix x){
  ...
  for( i=0; i<x.r; i++ ){ //can't load everything in one fread
    ...
    numRead = fread( &x.mat[IDX(i, 0, x.ld)], sizeof(float), x.c, fp);
    if(numRead != x.c){
...
void evalApprox(matrix q, matrix x, unint *NNs){
  float *ranges = (float*)calloc(q.pr, sizeof(*ranges));
...
void writeNeighbs(char *file, char *filetxt, unint **NNs, float **dNNs){
  unint i,j;
...
void saveRBC( matrix *R, rep *ri, char *filename ){
  ...
  for( i=0; i<nr; i++ ){
    ...
    safeWrite( ri[i].dists, sizeof(float), len, fp );
    
    safeWrite( &(ri[i].start), sizeof(unint), 1, fp );
    safeWrite( &(ri[i].radius), sizeof(float), 1, fp );
  }
  
  safeWrite( &(R->r), sizeof(unint), 1, fp );
  safeWrite( &(R->c), sizeof(unint), 1, fp );
  safeWrite( R->mat, sizeof(float), sizeOfMat(*R), fp );
...
void loadRBC( matrix *R, rep **ri, char* filename ){
  ...
  for( i=0; i<nr; i++ ){
    ...
    (*ri)[i].dists = (float*)calloc( plen, sizeof(float) );
    
    (*ri)[i].len = len;

    safeRead( (*ri)[i].lr, sizeof(unint), len, fp );
    safeRead( (*ri)[i].dists, sizeof(float), len, fp );
    safeRead( &((*ri)[i].start), sizeof(unint), 1, fp );
    safeRead( &((*ri)[i].radius), sizeof(float), 1, fp );
  }
  ...
  initMat( R, r, c );
  R->mat = (float*)calloc( sizeOfMat(*R), sizeof(float) );
  safeRead( R->mat, sizeof(float), sizeOfMat(*R), fp );
